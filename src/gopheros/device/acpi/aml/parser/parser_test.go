...
func TestResolveSymRefsErrors(t *testing.T) {
	root := entity.NewScope(entity.OpScope, 42, `\`)
	reg0 := entity.NewRegion(42)
	reg0.SetArg(0, "REG0")
	root.Append(reg0)
	root.Append(entity.NewFieldUnit(42, "FLD0"))

	specs := []entity.Entity{
		&entity.Field{RegionName: "MISSING"},
		&entity.IndexField{IndexRegName: "UNKNOWN"},
		&entity.IndexField{IndexRegName: "FLD0", DataRegName: "UNKNOWN"},
		&entity.BankField{RegionName: "MISSING"},
		&entity.BankField{RegionName: "REG0", BankFieldUnitName: "UNKNOWN"},
		&entity.FieldUnit{ConnectionName: "MISSING"},
		&entity.Reference{TargetName: "MISSING"},
		&entity.Invocation{MethodName: "UNKNOWN"},
	}

	for specIndex, spec := range specs {
		root.Append(spec)
		if resolveSymbolRefs(ioutil.Discard, spec, root) {
			t.Errorf("[spec %d] expected resolveReferences to return false", specIndex)
		}
	}
}
